...
MaterialInfo getMaterialInfo() {
	...
	vec2 mrSample = (uHasMetallicRoughnessTexture == 1) ? texture(uMetallicRoughnessTexture, texCoord).gb : vec2(1.);
...
void main() {
	...
	if (uHasEmissiveTexture == 1) {
		color += (texture(uEmissiveTexture, texCoord) * vec4(uEmissiveFactor, 1.0)).rgb;
	}
	color = clamp(color, 0.0, 1.0);
	color = mix(color, color * ((uHasOcclusionTexture == 1) ? texture(uOcclusionTexture, texCoord).r : 1.), 1.0);
