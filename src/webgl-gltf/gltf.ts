...
const getAccessor = (gltf: GlTf, mesh: gltf.Mesh, attributeName: string) => {
	const attribute = mesh.primitives[0].attributes[attributeName]
	return gltf.accessors![attribute]
}

const getBufferFromName = (gl: GLContext, gltf: GlTf, buffers: ArrayBuffer[], mesh: gltf.Mesh, name: string) => {
...
const loadAnimation = (gltf: GlTf, animation: gltf.Animation, buffers: ArrayBuffer[]) => {
...
const loadMesh = (gl: GLContext, gltf: GlTf, mesh: gltf.Mesh, buffers: ArrayBuffer[]) => {
 ...
 */
export const loadModel = async (gl: GLContext, uri: string) => {
	...
	const materials = gltf.materials
		? await Promise.all(gltf.materials.map(async (m) => await loadMaterial(gl, m, uri, gltf.images)))
		: []
